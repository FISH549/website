...
If you want to place the float at *exactly the location* in the **LaTeX** code, you will need to include a call to the `float` package in the `YAML` and then set the `fig.pos = "!H"` chunk option in the code block.

For example, set up the YAML with something like
...
and then later in the document use something like
...
Creating the **References** section is straightforward with `{knitcitations}`. All you have to do is call the function `write.bibtex()` and give it the name of the `.bib` file to which you would like the references written. The function will then create the reference cited section with the formatting specified in the `.cls` file supplied in the document's YAML.
